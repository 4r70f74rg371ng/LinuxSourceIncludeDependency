...
void estrcpy(char* target, char* source) {
	int i, top = 0;
	for(i=0;source[i];i++){
		if(source[i] == '\\'){
	...
	if (graph && filename) {
		...
		if (fp) {
			...
			Foreach_IncludeLinks(&(graph->links), ptr) {
				estrcpy(escapedpath, GetNodeFullPath(graph, ptr->start));
				estrcpy(escapedfile, GetNodeFullPath(graph, ptr->end));
			...
			Foreach_FileNodes(&(graph->nodes), ptr) {
				...
				estrcpy(escapedpath, fullpath);
				estrcpy(escapedfile, pointer);
